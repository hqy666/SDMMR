...
class Coach:
    ...
    def prepareModel(self):
        ...
        in_dims = out_dims[::-1]
        self.denoise_model_image = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm,
                                           mask_BCE=args.mask_BCE).cuda()
        self.denoise_opt_image = torch.optim.Adam(self.denoise_model_image.parameters(), lr=args.lr, weight_decay=0)

        out_dims = eval(args.dims) + [args.item]
        in_dims = out_dims[::-1]
        self.denoise_model_text = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm,
                                          mask_BCE=args.mask_BCE).cuda()
        ...
        if args.data == 'tiktok':
            out_dims = eval(args.dims) + [args.item]
            in_dims = out_dims[::-1]
            self.denoise_model_audio = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm,
                                               mask_BCE=args.mask_BCE).cuda()
    ...
    def trainEpoch(self):
        ...
        with torch.no_grad():
            ...
            for batch_in, batch in enumerate(diffusionLoader):
                ...
                denoised_batch = self.diffusion_model.p_sample(self.denoise_model_text, batch_item, args.sampling_steps,
                                                               args.sampling_noise, maskedBatch)
                top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)

                for i in range(batch_idx.shape[0]):
                    for j in range(indices_[i].shape[0]):
                        u_list_text.append(int(batch_idx[i].cpu().numpy()))
                ...
                if args.data == 'tiktok':
                    ...
                    for i in range(batch_idx.shape[0]):
                        for j in range(indices_[i].shape[0]):
                            u_list_audio.append(int(batch_idx[i].cpu().numpy()))
        ...
        with torch.no_grad():
            # image
            matrix = self.image_UI_matrix.to_dense()
            zero_matrix = torch.zeros_like(matrix)
            mask = (torch.rand(matrix.shape[0]) + args.keepRate).floor().type(torch.bool)
            zero_matrix[:, mask] = matrix[:, mask]
            self.image_UI_matrix = zero_matrix

            # text
            matrix = self.text_UI_matrix.to_dense()
            zero_matrix = torch.zeros_like(matrix)
            mask = (torch.rand(matrix.shape[0]) + args.keepRate).floor().type(torch.bool)
            zero_matrix[:, mask] = matrix[:, mask]
            self.text_UI_matrix = matrix

            if args.data == 'tiktok':
                # audio
                matrix = self.audio_UI_matrix.to_dense()
                zero_matrix = torch.zeros_like(matrix)
                mask = (torch.rand(matrix.shape[0]) + args.keepRate).floor().type(torch.bool)
                zero_matrix[:, mask] = matrix[:, mask]
                self.audio_UI_matrix = zero_matrix
