...
class GaussianDiffusion(nn.Module):
	...
	def de_noise(self, noisy_latents, model_pred, t):
	...
	def step_regularize(self, origin, latents, mean_reg_variation, sqrt_one_minus_alpha_prod, decay=0.01):

		noise = torch.randn_like(origin) / math.sqrt(
			origin.shape[1]
		)
		noise = noise * sqrt_one_minus_alpha_prod
		grad, = torch.autograd.grad(
			outputs=(origin * noise).sum(), inputs=latents, create_graph=True
		)
		reg_variations = torch.sqrt(grad.pow(2).sum(1))
	...
	def training_losses(self, model, x_start, itmEmbeds, batch_index, model_feats, batchMask=None):
		...
		origin, sqrt_one_minus_alpha_prod = self.de_noise(x_t, model_output, ts)
		mean_reg_variation = 0  # need to be manually resumed
		reg_loss, mean_reg_variation, reg_variations = self.step_regularize(
			origin, noise, mean_reg_variation, sqrt_one_minus_alpha_prod
		)

		diff_loss = mes_loss + args.lambda_reg * reg_loss
